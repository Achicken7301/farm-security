    ...
    case TS_RECEIVE:
    {
        ...
        for (int i = 0; i < max_socks; ++i)
        {
            if (sock[i] != INVALID_SOCK)
            {
                ...
                {
                    char data_echo_back[] =
                        "set ssid <your-ssid> - to set router ssid\nset "
                        "password <your-password> - to set router "
                        "password\nset apply settings- to apply above "
                        "setting\r\n";
                    /* PROCESS DATA THROUGH THIS */
                    TcpMess_Err_t tcpMesh_err =
                        process_rx_buffer(rx_buffer, sizeof(rx_buffer));

                    switch (tcpMesh_err)
                    {
                    case TCP_MESH_OK:
                    {
                        socket_send(TCP_TAG, sock[i],
                                    "Server receive success\r\n",
                                    sizeof("Server receive success\r\n"));
                        break;
                    }
                    case TCP_MESH_APPLY:
                    {
                    ...
                    default:
                        socket_send(TCP_TAG, sock[i], data_echo_back,

...
#endif
#if !USE_STA
    /* Mesh need this to scan for leaves, BUT when init
     * station mode, we already use this. SO that use #if
     * #endif here for */
  ...
  case MESH_RECEIVE:
  {
    ...
    if (pending.toSelf > 0 || pending.toDS > 0)
    {
      ...
      if (err == ESP_OK)
      {
        ...
        break;
        default:
          ESP_LOGI(MESH_TAG, "New data protocol: 0x%x", rx_data.proto);
                        ...
                        void *event_data)
{
  ...
  case MESH_EVENT_ROUTING_TABLE_ADD:
  {
    ...
    ESP_LOGW(MESH_TAG,
             "<MESH_EVENT_ROUTING_TABLE_ADD>add %d, "
             "new:%d, layer:%d",
  ...
  case MESH_EVENT_PARENT_CONNECTED:
  {
    ...
    ESP_LOGI(MESH_TAG,
             "<MESH_EVENT_PARENT_CONNECTED>layer:%d-->%d, "
             "parent:" MACSTR "%s, ID:" MACSTR ", duty:%d",
  ...
  case MESH_EVENT_FIND_NETWORK:
  {
    ...
    ESP_LOGI(MESH_TAG,
             "<MESH_EVENT_FIND_NETWORK>new channel:%d, "
             "router BSSID:" MACSTR "",

...
void fsm_sta()
{
  ...
  case MESH_SOCKET_INIT:
  {
    ...
    ESP_LOGI(MESH_TAG, "Socket created, connecting to %s:%d", host_ip, PORT);
  ...
  case MESH_SOCKET_RECEIVE:
  {
    ...
    else
    {
      ...
      if (process_server_data(rx_buffer) == TCP_MESH_OK)
      {
...
MeshError_t root_sendImage2server()
{
  ...
  if (err < 0)
  {
    ESP_LOGE(FSM_CAMERA_TAG, "FAIL SENDING last package: %d, Erro: 0x%x", package, err);
    return MESH_ROOT_SEND2SERVER_FAIL;
  }
                      ...
                      void *event_data)
{
  ...
  case IP_EVENT_STA_GOT_IP:
  {
    ...
    ESP_LOGI(IP_EVENT_HANDLER_TAG, "got ip:" IPSTR, IP2STR(&event->ip_info.ip));

    // set_cState(CAM_INIT);
  }
  break;
  default:
    ESP_LOGI(IP_EVENT_HANDLER_TAG, "event_base: %s\tevent_id%d", event_base, event_id);
    break;
  }
}
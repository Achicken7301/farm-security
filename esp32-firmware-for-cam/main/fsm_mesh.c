...
void fsm_mesh()
{
    ...
    case MESH_SEND_IMAGE:
    {
        ...
        uint8_t payload[50];
        int payload_size = sprintf((char *)payload, "%d", pic->len);
        mesh_tx_data.proto = MESH_PROTO_HTTP;
        mesh_tx_data.data = (uint8_t *)payload;
        mesh_tx_data.size = payload_size;
        mesh_send_err =
            esp_mesh_send(NULL, &mesh_tx_data, MESH_DATA_P2P, NULL, 0);
        ESP_LOGI(MESH_TAG, "Mesh Error Code 0x%02x",
                 (mesh_send_err == ESP_OK) ? ESP_OK : mesh_send_err);

        /* Send Image Data */
        ESP_LOGI(MESH_TAG, "%d/%d=%d packages, %d bytes left", pic->len,
                 MESH_MAX_TX_BUFF, pic->len / MESH_MAX_TX_BUFF, bytes_left);
        ...
        ESP_LOG_BUFFER_HEX(MESH_TAG, mesh_tx_data.data, 32);
        mesh_send_err =
            esp_mesh_send(NULL, &mesh_tx_data, MESH_DATA_P2P, NULL, 0);
        ESP_LOGI(MESH_TAG, "Mesh Error Code 0x%02x",
                 (mesh_send_err == ESP_OK) ? ESP_OK : mesh_send_err);

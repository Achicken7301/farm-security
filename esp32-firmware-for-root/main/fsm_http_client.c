 ...
 * @brief
 *
 * @param state HTTP_CLIENT_INIT,
 * @param state HTTP_CLIENT_DEINIT,
 * @param state HTTP_CLIENT_GET,
 * @param state HTTP_CLIENT_DO_NOTHING,
 * @param state HTTP_CLIENT_POST,
 */
...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
  ...
  case HTTP_EVENT_ON_HEADER:
    ESP_LOGD(HTTP_CLIENT_TAG, "HTTP_EVENT_ON_HEADER, key=%s, value=%s", evt->header_key,
             evt->header_value);
    break;
  case HTTP_EVENT_ON_DATA:
    ESP_LOGD(HTTP_CLIENT_TAG, "HTTP_EVENT_ON_DATA, len=%d", evt->data_len);
    /*
     *  Check for chunked encoding is added as the URL for
     * chunked encoding used in this example returns binary
     * data. However, event handler can also be used in case
    ...
    if (!esp_http_client_is_chunked_response(evt->client))
    {
      ...
      else
      {
        if (output_buffer == NULL)
        {
          ...
          if (output_buffer == NULL)
          {
            ESP_LOGE(HTTP_CLIENT_TAG, "Failed to allocate memory for output "
                                      "buffer");
            return ESP_FAIL;
          }
        }
  ...
  case HTTP_EVENT_ON_FINISH:
    ...
    if (output_buffer != NULL)
    {
      ...
      // ESP_LOG_BUFFER_HEX(HTTP_CLIENT_TAG, output_buffer,
      // output_len);
      free(output_buffer);
      output_buffer = NULL;
    }
...
void fsm_http_client()
{
  ...
  {
    /**
     * NOTE: All the configuration parameters for
     * http_client must be spefied either in URL or as host
     * and path parameters. If host and path parameters are
     * not set, query parameter will be ignored. In such
     * cases, query parameter should be specified in URL.
     *
     * If URL as well as host and path parameters are
     * specified, values of host and path will be
     * considered.
     */
  ...
  case HTTP_CLIENT_POST:
  {
    ...
    if (err == ESP_OK)
    {
      ESP_LOGI(HTTP_CLIENT_TAG, "HTTP POST Status = %d, content_length = %d",
